...
Route::middleware([\Modules\Web\Http\Middleware\ResolveVendor::class])->group(function () {
    ...
    Route::get('/promotions', [ProductDisplayController::class, 'promotions'])->name('web.promotions.index');
    Route::get('/products/{id}', [ProductDisplayController::class, 'show'])->name('web.products.show');
    Route::get('/promotions/{id}', [ProductDisplayController::class, 'showPromotion'])->name('web.promotions.show');
    ...
    // Regions API
    Route::get('/api/governorates/{governorate}/regions', function (\App\Models\Governorate $governorate, \Illuminate\Http\Request $request) {
        $vendorIdentifier = $request->query('vendor_id');
        $query = $governorate->regions()->where('is_active', true);

        if ($vendorIdentifier) {
            $vendor = \App\Models\Vendor::where('id', $vendorIdentifier)
