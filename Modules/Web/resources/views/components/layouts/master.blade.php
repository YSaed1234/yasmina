                ...
                <div class="flex justify-between h-20 items-center gap-x-4 lg:gap-x-12">
                    ...
                    <div class="flex items-center gap-4 xl:gap-8">
                        ...
                        <div class="flex xl:hidden items-center gap-0.5">
                            ...
                            @auth
                                <div class="relative" x-data="{ 
                                    ...
                                    @php 
                                        ...
                                        $unreadCount = auth()->user()->vendorUnreadNotifications($vendorId)->count(); 
                                    @endphp
                                    <button @click="open = !open" class="p-2 text-gray-500 hover:text-primary transition-colors relative">
                                    ...
                                    <div x-show="open" 
                                         ...
                                         class="fixed left-4 right-4 top-20 bg-white rounded-2xl shadow-2xl border border-yasmina-50 z-[60] overflow-hidden max-h-[70vh] flex flex-col" style="display: none;" x-cloak>
                                        <div class="p-4 border-b border-yasmina-50 flex justify-between items-center bg-yasmina-50/10">
                                            <span class="font-bold text-gray-900 text-sm">{{ __('Notifications') }}</span>
                                                ...
                                                <button @click="markRead('{{ $notification->id }}', '{{ $notification->data['action_url'] ?? route('web.notifications', ['vendor_id' => request('vendor_id')]) }}')" 
                                                   class="w-full text-start p-4 border-b border-yasmina-50/50 hover:bg-yasmina-50/30 transition-all block">
                                                    <div class="flex gap-3">
                                                        <div class="mt-1 w-2 h-2 rounded-full shrink-0 {{ $notification->read_at ? 'bg-gray-300' : 'bg-primary' }}"></div>
                                                        <div class="flex-1">
                                                            <p class="text-xs font-bold text-gray-800 leading-snug {{ $notification->read_at ? 'font-medium text-gray-500' : 'font-bold text-gray-900' }}">{{ $notification->data['message'] ?? '' }}</p>
                                                            <span class="text-[10px] text-gray-400 mt-1 block font-bold">{{ $notification->created_at->diffForHumans() }}</span>
                                                        </div>
                                                    </div>
                                                </button>
                                        ...
                                        <a href="{{ route('web.notifications', ['vendor_id' => request('vendor_id')]) }}" class="block py-4 text-center text-xs font-bold text-primary bg-yasmina-50/20 hover:bg-yasmina-50/50 transition-all border-t border-yasmina-50">{{ __('View All') }}</a>
                                    </div>
                                </div>
                            @endauth
                            <!-- Shop -->
                            ...
                            @auth
                                <!-- Profile -->
                                <a href="{{ route('web.profile', ['vendor_id' => request('vendor_id')]) }}" class="p-2 text-gray-500 hover:text-primary transition-colors">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="absolute top-0 {{ app()->getLocale() == 'ar' ? 'right-0' : 'left-0' }} w-[85%] sm:w-80 bg-white shadow-2xl flex flex-col h-auto max-h-[90vh] rounded-b-3xl border-{{ app()->getLocale() == 'ar' ? 'l' : 'r' }} border-b border-yasmina-50 overflow-hidden">
                    ...
                    <div class="p-4 border-b border-yasmina-50 flex items-center justify-between bg-white">
                        ...
                        <div class="flex items-center gap-2">
                            <!-- Theme Toggle -->
                            <button onclick="let t = localStorage.getItem('yasmina-theme') === 'barbie' ? 'yasmina' : 'barbie'; setTheme(t); location.reload();" class="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-yasmina-50/50 text-gray-600 transition-all">
                                <div class="w-4 h-4 rounded-full border-2 border-white shadow-sm" 
                                     style="background: {{ (isset($_COOKIE['yasmina-theme']) && $_COOKIE['yasmina-theme'] == 'barbie') ? '#865d58' : '#e0218a' }}">
                                </div>
                                <span class="text-[10px] font-bold">{{ __('Theme') }}</span>
                            </button>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12 text-center md:text-left">
                        ...
                        @if($currentVendor)
                            <div class="flex items-center justify-center md:justify-start gap-3">
                                @if($currentVendor->facebook)
                                    <a href="{{ $currentVendor->facebook }}" target="_blank" class="w-8 h-8 rounded-full flex items-center justify-center text-gray-400 hover:bg-primary hover:text-white transition-all" style="background-color: var(--yasmina-footer-item)">
                                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                                    </a>
                                @endif
                                @if($currentVendor->instagram)
                                    <a href="{{ $currentVendor->instagram }}" target="_blank" class="w-8 h-8 rounded-full flex items-center justify-center text-gray-400 hover:bg-primary hover:text-white transition-all" style="background-color: var(--yasmina-footer-item)">
                                ...
                                @if($currentVendor->twitter)
                                    <a href="{{ $currentVendor->twitter }}" target="_blank" class="w-8 h-8 rounded-full flex items-center justify-center text-gray-400 hover:bg-primary hover:text-white transition-all" style="background-color: var(--yasmina-footer-item)">
                                ...
                                @if($currentVendor->whatsapp)
                                    <a href="https://wa.me/{{ $currentVendor->whatsapp }}" target="_blank" class="w-8 h-8 rounded-full flex items-center justify-center text-gray-400 hover:bg-primary hover:text-white transition-all" style="background-color: var(--yasmina-footer-item)">
                    ...
                    <div class="text-start">
                        <h4 class="text-sm lg:text-lg font-bold mb-4 lg:mb-6 uppercase tracking-wider">{{ __('Quick Links') }}</h4>
                        <ul class="space-y-2 lg:space-y-4 text-gray-400 text-[10px] lg:text-sm">
                    ...
                    <div class="text-start">
                        <h4 class="text-sm lg:text-lg font-bold mb-4 lg:mb-6 uppercase tracking-wider">{{ __('Contact Info') }}</h4>
                        <ul class="space-y-2 lg:space-y-4 text-gray-400 text-[10px] lg:text-sm">
                            @if($currentVendor)
                                ...
                                @endif
                                <li class="flex items-center gap-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                                    {{ $currentVendor->email }}
                                </li>
                            @else
                                <li class="flex items-center gap-2">
                    ...
                    <div class="col-span-2 lg:col-span-1">
                        <h4 class="text-sm lg:text-lg font-bold mb-4 lg:mb-6 uppercase tracking-wider">{{ __('Newsletter') }}</h4>
                        <form class="flex max-w-sm mx-auto md:mx-0">
                            <input type="email" placeholder="{{ __('Email') }}" class="border-none rounded-l-2xl px-4 py-2 w-full focus:ring-1 focus:ring-primary outline-none text-white text-[10px] lg:text-sm" style="background-color: var(--yasmina-footer-item)">
                            <button type="button" class="bg-primary rounded-r-2xl px-4 py-2 font-bold hover:opacity-90 transition-colors text-[10px] lg:text-sm">{{ __('Join') }}</button>
                        </form>
                    </div>
                </div>
                <div class="mt-10 lg:mt-20 pt-8 text-center text-gray-500 text-[10px] lg:text-sm" style="border-top: 1px solid var(--yasmina-footer-border)">
