        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-10">
            ...
            <form action="{{ route('admin.users.update', $user) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                ...
                <div class="col-span-full">
                    ...
                    @if($user->role == 'admin')
                        <div class="p-8 bg-primary/5 border-2 border-primary/20 rounded-3xl flex items-center gap-6">
                            <div class="w-16 h-16 rounded-2xl bg-primary flex items-center justify-center text-white shadow-lg shadow-primary/20">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                                </svg>
                            </div>
                            <div>
                                <h4 class="text-xl font-bold text-gray-900 mb-1">{{ __('System Administrator') }}</h4>
                                <p class="text-sm text-gray-500">{{ __('This user has full system access. Role modification is restricted for safety.') }}</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($roles as $role)
                                <label class="relative cursor-pointer group h-full">
                                    <input type="radio" name="role" value="{{ $role->name }}" class="peer hidden" {{ $user->hasRole($role->name) ? 'checked' : '' }} required>
