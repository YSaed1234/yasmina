<x-admin::layouts.master>
    <div class="mb-10">
        ...
        <h1 class="text-3xl font-bold text-gray-900 tracking-tight">{{ __('Edit Slide') }}</h1>
        <p class="text-gray-500 mt-2">{{ __('Modify the content and style for your hero slide.') }}</p>
    </div>

    <form action="{{ route('admin.slides.update', $slide->id) }}" method="POST" enctype="multipart/form-data" class="max-w-4xl space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="md:col-span-2">
                    ...
                    <div class="flex items-center gap-6 p-6 bg-gray-50 rounded-3xl border border-gray-100 mb-4">
                        <div class="w-40 h-20 rounded-xl overflow-hidden shadow-sm">
                            <img src="{{ asset('storage/' . $slide->image) }}" class="w-full h-full object-cover">
                        </div>
                        <div class="text-sm text-gray-500">{{ __('Current image') }}</div>
                    </div>
                ...
                <div>
                    ...
                    <input type="text" name="title_ar" value="{{ $slide->title_ar }}" required class="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{{ __('Title (English)') }}</label>
                    <input type="text" name="title_en" value="{{ $slide->title_en }}" required class="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium">
                </div>

                <div>
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{{ __('Subtitle (Arabic)') }}</label>
                    <input type="text" name="subtitle_ar" value="{{ $slide->subtitle_ar }}" class="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{{ __('Subtitle (English)') }}</label>
                    <input type="text" name="subtitle_en" value="{{ $slide->subtitle_en }}" class="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium">
                </div>

                <div class="md:col-span-2">
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{{ __('Description (Arabic)') }}</label>
                    <textarea name="description_ar" rows="3" class="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium">{{ $slide->description_ar }}</textarea>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{{ __('Description (English)') }}</label>
                    <textarea name="description_en" rows="3" class="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium">{{ $slide->description_en }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="text" name="button_text_ar" value="{{ $slide->button_text_ar }}" class="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{{ __('Button Text (English)') }}</label>
                    <input type="text" name="button_text_en" value="{{ $slide->button_text_en }}" class="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium">
                </div>
                <div class="md:col-span-2">
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{{ __('Button Link (URL)') }}</label>
                    <input type="text" name="link" value="{{ $slide->link }}" placeholder="https://..." class="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium text-left text-ltr">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[2.5rem] p-10 shadow-sm border border-gray-100">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="number" name="order" value="{{ $slide->order }}" class="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium">
                </div>
                <div>
                    ...
                    <select name="active" class="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none transition-all font-medium appearance-none">
                        <option value="1" {{ $slide->active ? 'selected' : '' }}>{{ __('Active') }}</option>
