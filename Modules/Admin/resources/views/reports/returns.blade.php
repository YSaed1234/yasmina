<x-admin::layouts.master>
    <div class="mb-10 flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-500 mt-2">{{ __('Track and analyze product return requests across all institutions.') }}</p>
        </div>
        
        @if(!auth()->user()->vendor_id)
        <div class="flex items-center gap-4">
            <form action="{{ route('admin.reports.returns') }}" method="GET" class="flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-12">
        ...
        @if($returnStats->isEmpty())
             <div class="col-span-4 bg-white p-12 rounded-3xl border border-dashed border-gray-200 text-center text-gray-400 italic">
                {{ __('No return data available for the selected filter.') }}
            </div>
        @endif
    </div>
                ...
                @forelse($recentReturns as $request)
                    <tr class="hover:bg-gray-50/50 transition-all group">
                        ...
                        <td class="px-8 py-6 text-right">
                            <a href="{{ route('admin.returns.show', $request) }}" class="text-primary font-bold text-sm hover:underline">
                                {{ __('View Details') }}
                            </a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="px-8 py-20 text-center text-gray-400 italic">
                            {{ __('No recent return requests found.') }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
