<x-admin::layouts.master>
    <div class="mb-10">
        ...
        <h1 class="text-3xl font-bold text-gray-900 tracking-tight">{{ __('Add New Driver') }}</h1>
        <p class="text-gray-500 mt-2">{{ __('Create a new delivery driver profile.') }}</p>
    </div>

    <form action="{{ route('admin.drivers.store') }}" method="POST" class="max-w-4xl">
        @csrf
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-10 space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none font-bold text-gray-700 transition-all">
                ...
                <div>
                    ...
                    <input type="text" name="phone" value="{{ old('phone') }}" required class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none font-bold text-gray-700 transition-all">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none font-bold text-gray-700 transition-all">
            ...
            <div class="pt-8 border-t border-gray-50 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="text" name="vehicle_type" value="{{ old('vehicle_type') }}" placeholder="e.g. Motorcycle, Van" class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none font-bold text-gray-700 transition-all">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{{ __('Vehicle Number') }}</label>
                    <input type="text" name="vehicle_number" value="{{ old('vehicle_number') }}" class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none font-bold text-gray-700 transition-all">
                </div>
            </div>
