<x-admin::layouts.master>
    <div class="mb-10 flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 tracking-tight">{{ __('Regions & Shipping') }} <span class="ml-2 px-3 py-1 bg-primary/10 text-primary text-sm rounded-full">{{ $regions->total() }}</span></h1>
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse($regions as $region)
                    <tr class="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td class="px-8 py-6">
                            @if($region->is_active)
                                <span class="px-3 py-1 bg-green-50 text-green-600 rounded-full text-[10px] font-black uppercase tracking-widest">
                                    {{ __('Active') }}
                                </span>
                            @else
                                <span class="px-3 py-1 bg-red-50 text-red-600 rounded-full text-[10px] font-black uppercase tracking-widest">
                                    {{ __('Inactive') }}
                                </span>
                            @endif
                        </td>
