<x-admin::layouts.master>
    <div class="flex justify-between items-center mb-10">
        ...
        <a href="{{ route('currencies.create') }}" class="px-6 py-3 bg-barbie-500 text-white rounded-2xl font-bold hover:bg-barbie-600 transition-all shadow-lg shadow-barbie-100 flex items-center gap-2">
        ...
        <table class="min-w-full divide-y divide-barbie-50">
            <thead>
                <tr class="bg-barbie-50/50">
                    ...
                    <th class="px-8 py-5 text-left text-xs font-bold text-barbie-500 uppercase tracking-widest">{{ __('Code') }}</th>
                    <th class="px-8 py-5 text-left text-xs font-bold text-barbie-500 uppercase tracking-widest">{{ __('Symbol') }}</th>
                ...
                @foreach($currencies as $currency)
                    <tr class="hover:bg-barbie-50/30 transition-colors">
                        <td class="px-8 py-5 whitespace-nowrap text-sm font-bold text-gray-800">
                            {{ $currency->translate(app()->getLocale())->name ?? $currency->name }}
                        </td>
                        <td class="px-8 py-5 whitespace-nowrap font-mono text-barbie-500 font-bold tracking-wider">{{ $currency->code }}</td>
                        ...
                        <td class="px-8 py-5 whitespace-nowrap text-right text-sm font-bold">
                            <div class="flex justify-end gap-3">
                                <a href="{{ route('currencies.edit', $currency->id) }}" class="p-2 text-barbie-500 hover:bg-barbie-50 rounded-xl transition-all">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
