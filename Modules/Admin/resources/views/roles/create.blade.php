<x-admin::layouts.master>
    <div class="mb-10">
        <h1 class="text-3xl font-bold text-gray-800">{{ __('Create New Role') }}</h1>
        <p class="text-gray-500 mt-2">{{ __('Define permissions for a new access level.') }}</p>
    </div>
    ...
    <div class="max-w-4xl bg-white/70 backdrop-blur-md p-8 rounded-3xl border border-yasmina-50 shadow-xl shadow-yasmina-100/50">
        <form action="{{ route('roles.store') }}" method="POST" class="space-y-8">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" class="w-full px-5 py-4 bg-yasmina-50/50 border border-yasmina-100 rounded-2xl focus:bg-white focus:ring-4 focus:ring-yasmina-100 focus:border-yasmina-300 transition-all outline-none font-bold text-gray-700" placeholder="{{ __('e.g. Editor') }}" required>
                    ...
                    <div class="bg-yasmina-50/30 p-6 rounded-3xl border border-yasmina-50 permission-group">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <label class="flex items-center gap-3 p-4 bg-white border border-yasmina-50 rounded-2xl hover:bg-yasmina-50 transition-all cursor-pointer group shadow-sm">
                                <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" class="w-5 h-5 text-yasmina-500 border-yasmina-200 rounded focus:ring-yasmina-500 transition-all">
