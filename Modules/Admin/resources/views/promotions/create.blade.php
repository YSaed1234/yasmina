<x-admin::layouts.master>
    <div class="max-w-4xl mx-auto" x-data="{ 
        ...
        <div class="mb-10 flex items-center justify-between">
            <div>
                <h1 class="text-4xl font-bold text-gray-900 mb-2">{{ __('Create Promotion') }}</h1>
                <p class="text-gray-500">{{ __('Configure a new BOGO or bundle deal.') }}</p>
            </div>
        ...
        <form action="{{ route('admin.promotions.store') }}" method="POST" class="space-y-8">
            @csrf
            ...
            <div class="bg-white p-10 rounded-[2.5rem] shadow-sm border border-yasmina-50">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    <div>
                        ...
                        <input type="text" name="name_ar" class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-primary outline-none transition-all" placeholder="مثلاً: عرض اشتري واحد واحصل على واحد مجاناً">
                    </div>
                    <div>
                        <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3">{{ __('Promotion Name (English)') }}</label>
                        <input type="text" name="name_en" class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-primary outline-none transition-all" placeholder="e.g., Buy 1 Get 1 Free Deal">
                    </div>
                </div>
                ...
                <div class="mb-8">
                    ...
                    <p class="mt-2 text-[10px] text-gray-400 font-bold uppercase tracking-widest italic">{{ __('If selected, this promotion will be linked to this institution.') }}</p>
                </div>
                                ...
                                <div>
                                    ...
                                    <select name="buy_product_id" class="w-full px-4 py-3 bg-white border border-blue-100 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all text-sm select2">
                                        @foreach($products as $product)
                                            <option value="{{ $product->id }}">{{ $product->name }} ({{ $product->vendor->name ?? 'N/A' }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">{{ __('Quantity required') }}</label>
                                    <input type="number" name="buy_quantity" value="1" min="1" class="w-full px-4 py-3 bg-white border border-blue-100 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all text-sm">
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="p-6 bg-yasmina-50/50 rounded-3xl border border-yasmina-100">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="get_quantity" value="1" min="1" class="w-full px-4 py-3 bg-white border border-yasmina-100 rounded-xl focus:ring-2 focus:ring-primary outline-none transition-all text-sm">
                                </div>
                                <div x-show="discountType !== 'free'">
                                    ...
                                    <div class="relative">
                                        <input type="number" step="0.01" name="discount_value" value="0" class="w-full px-4 py-3 bg-white border border-yasmina-100 rounded-xl focus:ring-2 focus:ring-primary outline-none transition-all text-sm">
            ...
            <div class="bg-white p-10 rounded-[2.5rem] shadow-sm border border-yasmina-50">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-end">
                    <div>
                        ...
                        <input type="datetime-local" name="starts_at" class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-primary outline-none transition-all">
                    </div>
                    <div>
                        <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-3">{{ __('Expires At') }}</label>
                        <input type="datetime-local" name="expires_at" class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-primary outline-none transition-all">
                    </div>
