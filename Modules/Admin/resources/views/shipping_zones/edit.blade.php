<x-admin::layouts.master>
    <div class="mb-10 flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 tracking-tight">{{ __('Edit Shipping Zone') }}</h1>
            <p class="text-gray-500 mt-2">{{ __('Update regional shipping rate.') }}</p>
        </div>
    ...
    <div class="max-w-2xl">
        <form action="{{ route('admin.shipping_zones.update', $shippingZone) }}" method="POST" class="bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100 space-y-8">
            @csrf
            @method('PUT')
            <div>
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">{{ __('Zone Name') }}</label>
                <input type="text" name="name" value="{{ old('name', $shippingZone->name) }}" required
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" step="0.01" name="rate" value="{{ old('rate', $shippingZone->rate) }}" required
                           class="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-primary/10 outline-none font-bold text-gray-700 transition-all">
            ...
            <div>
                <label class="flex items-center gap-3 cursor-pointer group">
                    <div class="relative">
                        <input type="checkbox" name="is_active" value="1" {{ $shippingZone->is_active ? 'checked' : '' }} class="sr-only peer">
