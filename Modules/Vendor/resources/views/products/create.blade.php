<x-vendor::layouts.master>
    <div class="mb-10">
        ...
        <p class="text-gray-500 mt-2">{{ __('List a new product for your institution.') }}</p>
    </div>

    <div class="max-w-4xl bg-white/70 backdrop-blur-md p-8 rounded-3xl border border-gray-100 shadow-xl">
        <form action="{{ route('vendor.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            @if ($errors->any())
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="text" name="ar[name]" class="w-full px-5 py-4 bg-gray-50/50 border border-gray-100 rounded-2xl focus:bg-white focus:ring-4 focus:ring-primary/10 focus:border-primary/30 transition-all outline-none font-bold text-gray-700" placeholder="{{ __('e.g. فستان حرير') }}" required>
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-400 mb-2 uppercase tracking-widest">{{ __('Product Name (English)') }}</label>
                    <input type="text" name="en[name]" class="w-full px-5 py-4 bg-gray-50/50 border border-gray-100 rounded-2xl focus:bg-white focus:ring-4 focus:ring-primary/10 focus:border-primary/30 transition-all outline-none font-bold text-gray-700" placeholder="{{ __('e.g. Silk Dress') }}" required>
                </div>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="price" class="w-full px-5 py-4 bg-gray-50/50 border {{ $errors->has('price') ? 'border-red-500' : 'border-gray-100' }} rounded-2xl focus:bg-white focus:ring-4 focus:ring-primary/10 focus:border-primary/30 transition-all outline-none font-bold text-gray-700" placeholder="0.00" value="{{ old('price') }}" required>
                    @error('price')
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="discount_price" class="w-full px-5 py-4 bg-gray-50/50 border {{ $errors->has('discount_price') ? 'border-red-500' : 'border-gray-100' }} rounded-2xl focus:bg-white focus:ring-4 focus:ring-primary/10 focus:border-primary/30 transition-all outline-none font-bold text-gray-700" placeholder="0.00" value="{{ old('discount_price') }}">
                ...
                <div>
                    ...
                    <input type="number" name="stock" class="w-full px-5 py-4 bg-gray-50/50 border {{ $errors->has('stock') ? 'border-red-500' : 'border-gray-100' }} rounded-2xl focus:bg-white focus:ring-4 focus:ring-primary/10 focus:border-primary/30 transition-all outline-none font-bold text-gray-700" placeholder="0" value="{{ old('stock', 0) }}" required>
                    @error('stock')
                        <p class="mt-1 text-xs font-bold text-red-500 uppercase tracking-widest">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="p-6 bg-amber-50/50 rounded-3xl border border-amber-100 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="flash_sale_price" class="w-full px-5 py-4 bg-white border {{ $errors->has('flash_sale_price') ? 'border-red-500' : 'border-amber-100' }} rounded-2xl focus:ring-4 focus:ring-amber-100 outline-none font-bold text-gray-700" placeholder="0.00" value="{{ old('flash_sale_price') }}">
                ...
                <div class="md:col-span-1">
                    ...
                    <textarea name="en[description]" rows="4" class="w-full px-5 py-4 bg-gray-50/50 border border-gray-100 rounded-2xl focus:bg-white focus:ring-4 focus:ring-primary/10 focus:border-primary/30 transition-all outline-none font-bold text-gray-700" placeholder="{{ __('Product description...') }}"></textarea>
                </div>
