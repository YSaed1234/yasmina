...
<head>
    ...
    <title>{{ __('Vendor Login') }} | {{ config('app.name', 'Yasmina') }}</title>

    <!-- Fonts -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700&family=Tajawal:wght@400;700&display=swap" rel="stylesheet">
...
<body>
    ...
    <div class="login-card">
        <div class="text-center mb-12">
            ...
            <p class="text-xs uppercase tracking-[0.3em] text-gray-400 font-semibold">{{ __('Vendor Panel') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('vendor.login.submit') }}" class="space-y-6">
            ...
            <div class="input-group">
                ...
                <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus
                    class="block w-full px-5 py-4 bg-gray-50/50 border-none rounded-2xl focus:ring-2 focus:ring-primary/20 focus:bg-white transition-all text-gray-900 placeholder-gray-300"
            ...
            <div class="flex items-center justify-between pt-2">
                <label class="flex items-center group cursor-pointer">
                    <input type="checkbox" name="remember" class="w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary transition-all">
                    <span class="ml-2 text-sm text-gray-500 group-hover:text-gray-700 transition-colors">{{ __('Remember me') }}</span>
                </label>
            </div>

            <div class="pt-4">
                <button type="submit" class="w-full py-5 btn-primary text-white rounded-2xl font-bold text-lg shadow-xl shadow-gray-200">
                    {{ __('Sign In') }}
                </button>
            </div>
        </form>
        ...
        <div class="mt-12 text-center">
            <p class="text-sm text-gray-400">
                {{ __('Need help?') }} 
                <a href="mailto:support@yasmina.com" class="text-primary font-bold hover:underline ml-1">{{ __('Contact Support') }}</a>
            </p>
        </div>
    </div>
</body>
